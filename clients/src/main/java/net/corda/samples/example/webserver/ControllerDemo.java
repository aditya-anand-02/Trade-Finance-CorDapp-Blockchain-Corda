...
import net.corda.samples.example.flows.MakePayment;
...
public class ControllerDemo {
    ...
    public ControllerDemo(NodeRPCConnection rpc) {
    ...
    public HashMap<String, List<String>> getPeers() {
        ...
        // Find all nodes that are not notaries, ourselves, or the network map.
    ...
    @GetMapping(value = "/messagestates", produces = TEXT_PLAIN_VALUE)
    private String states2() {
        return proxy.vaultQuery(MessageState.class).getStates().toString();
    }
    ...
    public List<StateAndRef<MessageState>> getMessages() {
        // Filter by state type: IOU.
        return proxy.vaultQuery(MessageState.class).getStates();
    }
    ...
    public ResponseEntity<String> issueIOU(HttpServletRequest request) throws IllegalArgumentException {
        ...
        String party = request.getParameter("partyName");
         String currStatus = request.getParameter("statusUpdate");
        logger.info("Display problem 1");
        ...
        logger.info("Display Problem 2");

        // Create a new IOU state using the parameters given.
        try {
            // Start the IOUIssueFlow. We block and waits for the flow to return.
            SignedTransaction result = proxy.startTrackedFlowDynamic(MakePayment.Initiator.class, amount,otherParty, currStatus).getReturnValue().get();
            logger.info("DisplayProblem 3");
            // Return the response.
    ...
    @PostMapping (value = "create-message" , produces =  TEXT_PLAIN_VALUE , headers =  "Content-Type=application/x-www-form-urlencoded" )
    public ResponseEntity<String> issueMessage(HttpServletRequest request) throws IllegalArgumentException {
        ...
        // Create a new Initiator state using the parameters given.
        try {
            // Start the IOUIssueFlow. We block and waits for the flow to return.
           //SignedTransaction result = proxy.startTrackedFlowDynamic(MakePayment.Initiator.class, amount,otherParty).getReturnValue().get();
            SignedTransaction res=proxy.startTrackedFlowDynamic(SendMessage.Initiator.class, msg, otherParty1).getReturnValue().get();
            // Return the response.
            return ResponseEntity
                    .status(HttpStatus.CREATED)
                    .body("Transaction id "+ res.getId() +" committed to ledger.\n " + res.getTx().getOutput(0));
            // For the purposes of this demo app, we do not differentiate by exception type.
        } catch (Exception e) {
            return ResponseEntity
                    .status(HttpStatus.BAD_REQUEST)
    ...
    public ResponseEntity<List<StateAndRef<MessageState>>> getMyMessages() {
